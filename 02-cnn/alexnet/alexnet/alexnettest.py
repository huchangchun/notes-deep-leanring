...
mnist = input_data.read_data_sets('data\\', one_hot=True)#[-16:]
#28*28*1
...
def conv2d(name,input_data,input_filter,bias):      #[batch, in_height, in_width, in_channels],batch=1：一次只处理一张图片，in_channels=1：单通道
    x=tf.nn.conv2d(input_data,input_filter, strides=[1,1,1,1], padding='SAME', use_cudnn_on_gpu=False, 
                  data_format="NHWC", name=None)
    x=tf.nn.bias_add(x, bias, data_format=None, name=None) #加偏置
...
weights={
    ...
    'wc1': tf.Variable(tf.random_normal([11, 11, 1, 48])), #48个卷积核
...
#定义优化器反向传播，定义学习率，目标是最小化损失
optimizer = tf.train.AdamOptimizer(learning_rate=learn_rate).minimize(cost)   
# 测试网络
#tf.arg_max(feature,1)是按行取最大值的下标,假如下标一样，返回TRUE，否则返回False
correct_feature = tf.equal(tf.argmax(feature,1), tf.argmax(y,1))
#先将correct_feature中TRUE和false转换为float32类型,1,0
#求correct_feature中的平均值，因为correct_feature中除了0就是1，因此求平均值即为1的所占比例，即正确率
...
def train():
    with tf.Session() as sess:
        ...
        #print ('featureiction is:',np.where(np.int16(np.round(featureict[img_index,:],0))==1)[0][0])    
        ##可视化卷积核
    ...
    if os.path.exists("ckpt"):
        ...
        if count ==4:
            ...
            with tf.Session() as sess:
                ...
                a_img=np.reshape(img_input,[28,28])
                #把预测结果打印出来
